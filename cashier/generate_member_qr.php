...
try {
    ...
    if (!is_writable($dir)) {
        ob_end_clean();
        echo json_encode(['error' => 'qr_codes folder is not writable.']);
        exit();
    }
    ...
    if (!file_exists($tmp_file)) {
        ob_end_clean();
        echo json_encode(['error' => 'QR image was not created. Enable PHP GD extension.']);
        exit();
    }
    ...
    $font     = 5;
    $banner_h = 48;
    $footer_h = !empty($member_name) ? 36 : 0;
    $padding  = 16;
    ...
    if (!file_exists($filepath)) {
        ob_end_clean();
        echo json_encode(['error' => 'QR file could not be saved.']);
        exit();
    }

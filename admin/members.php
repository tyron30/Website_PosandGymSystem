...
// Ensure website_bookings columns exist before querying them
...
$query = "SELECT m.*,
          ...
          wb.id as website_booking_id,
          wb.gcash_ref as website_gcash_ref,
          verifier.fullname as verified_by_name,
          verifier.role as verified_by_role
          FROM members m
          LEFT JOIN users u ON m.created_by = u.id
          LEFT JOIN website_bookings wb ON wb.created_member_id = m.id
          LEFT JOIN users verifier ON verifier.id = wb.verified_by";
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if (!empty($member['website_booking_id'])): ?>
                                                        <span class="badge bg-primary" title="GCash Ref: <?php echo htmlspecialchars($member['website_gcash_ref'] ?? ''); ?>">
                                                            <i class="fas fa-globe me-1"></i>Online Booking
                                                        </span>
                                                    <?php else: ?>
                                                        <span class="badge bg-secondary">
                                                            <i class="fas fa-desktop me-1"></i>Gym System
                                                        </span>
                                                    <?php endif; ?>
                                                </td>
                                                ...
                                                <td>
                                                    <?php
                                                    if (!empty($member['website_booking_id'])) {
                                                        // Show who verified/accepted the online booking
                                                        $verifier = $member['verified_by_name'] ?? null;
                                                        $v_role   = $member['verified_by_role'] ?? '';
                                                        if ($verifier) {
                                                            $badge = $v_role === 'admin' ? 'bg-danger' : 'bg-success';
                                                            echo '<span class="badge ' . $badge . '">' . htmlspecialchars($verifier) . '</span>';
                                                        } else {
                                                            echo '<span class="text-muted small fst-italic">Pending</span>';
                                                        }
                                                    } else {
                                                        // Show who added the member via the gym system
                                                        $creator_name = $member['created_by_name'] ?? 'Unknown';
                                                        $role = $member['created_by_role'] ?? '';
                                                        if ($role === 'admin') {

...
const voices = {
  ...
  expired(name) {
    speak(`Access denied. Membership expired${name ? ' for ' + name : ''}. Please renew your membership.`);
  },
  unknown() {
  ...
  .then(data => {
    ...
    if (data.success) {
      ...
      if (lmsg.includes('expired')) {
        ...
        beeps.error();
        flash('#ef4444');
        showResult('error', name || 'Expired Member', msg, 'Please renew membership at the front desk.');
